...
function renderJob(job, ranked) {
  ...
  return `<a href="job-detail.html?job_id=${job.id}" class="glass block rounded-3xl border p-5">
    ...
    <div class="mt-4 flex items-center justify-between text-sm text-slate-500"><span>$${job.budget}</span><span>Suggested specialists: ${matches || 'Run search'}</span></div>
  </a>`;
}
...
function renderClientJob(job) {
  return `<a href="job-detail.html?job_id=${job.id}" class="glass block rounded-3xl border p-5">
    ...
    <p class="mt-3 text-sm text-slate-600">${job.description}</p>
    <div class="mt-4 text-sm text-slate-500">$${job.budget}</div>
  </a>`;
}
